...
## ----message=F, warning = F, results=F, include=F------------------------------------
# definiamo la directory di lavoro:
setwd("~/Desktop/Corso_R")

# Carichiamo i dati in R:
dd <- read.csv(text = RCurl::getURL(
"https://raw.githubusercontent.com/marcoplebani85/datasets/master/mixed_effect_data2.csv")
  )
# getURL() permette di accedere a dati online


## ----mem2, message=F, warning = F, results=F, fig.align="center", fig.width=6, fig.height=5, fig.cap="Dati e predizioni secondo una regressione lineare per ciascuna località."----
...
## ----warning = F, tidy=F-------------------------------------------------------------
mem1 <- lmerTest::lmer(Alga_length ~ Depth + (1 | Locality), 
                data = dd, REML=F)


## ----mem2diagn, warning = F, message=F, fig.align="center", fig.width=6.5, fig.height=5, fig.cap="Grafici diagnostici per $mem1$."----
...
## ----mem2diagn2, warning = F, message=F, fig.align="center", fig.width=6, fig.height=8, fig.cap="Grafici diagnostici per $mem1$ usando il pacchetto $performance$."----
...
## ----tidy=F--------------------------------------------------------------------------
mem1reml <- lmerTest::lmer(Alga_length ~ Depth + (1 | Locality), 
                data = dd, REML=T)
...
## ----mem2fixef, message=F, warning = F, results=F, tidy=F, fig.align="center", fig.width=6, fig.height=5, fig.cap="Predizioni di $mem1$."----
